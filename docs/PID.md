...
| 1.5 | 2026-05-25 | Domains 2, 3, 5 (PIS-15, PIS-18, PIS-21, PIS-25, Eval 1, Eval 4): TCO contract extended with **mid-cycle refresh events** and **version snapshots**. `TCOFormInputs` gains `refresh_events: list[RefreshEvent]` (default `[]`); each event carries `year` ∈ [2, 5] (Year 1 is the initial deployment), `percent_of_devices` ∈ (0, 100], and an optional `cost_per_unit_override`. The TCO engine adds a `refresh_hardware` line in the targeted year only — licensing, support, and adjacent recurring are unaffected because refreshed units re-use the existing fleet line. `YearCost` (PIS-15) gains a matching `refresh_hardware` field, defaulted to `0.0` so pre-amendment scenarios deserialize unchanged. PIS-21 reasonableness checks extended: refresh events totalling >100% of the fleet flagged; refresh events with `year > lifecycle_years` are flagged AND excluded from totals (not silently rolled in). PIS-24 #4 (5-year horizon) still bounds the model — refresh events are validated against the lifecycle, never beyond it. **Versioning** (PIS-25): `TCOScenario` gains `lineage_id` (an indexed grouping key, NOT a foreign key — a deleted version must not cascade through its lineage) and `version` (1-indexed). A new save with `parent_scenario_id` inherits the parent's lineage and bumps version; without it, a fresh lineage starts at v1 anchored to its own id. The existing `POST /projects/{id}/tco` route accepts the optional `parent_scenario_id` field — no new write endpoint. A read-only convenience route `GET /projects/{id}/tco/lineages/{lineage_id}` returns the full version history scoped to the project (SEC-27). The Report schema (`ReportArtifact`) gains a new kind `tco_comparison` with paired `ref_id` + `ref_id_b` for side-by-side PDF exports; Pydantic validates that both ids are set and distinct. PDF renderer surfaces the version label and save date on every TCO scenario header. Eval 1 figures unchanged ($218,000 5-year total) — empty `refresh_events` is the default. Backward compatibility: scenarios saved before this amendment load unchanged, and the Alembic migration backfills `lineage_id = id`, `version = 1` for all existing rows. Approved at start of refresh + versioning work. |
...
| 11 | 2026-06-06 | 3 | garak authed with a 401 against NIM even though `NVIDIA_API_KEY` was set | garak's NIM generator reads the key from **`NIM_API_KEY`**, not `NVIDIA_API_KEY` (the var the rest of the engagement uses) | Run script exports `NIM_API_KEY="$NVIDIA_API_KEY"` before invoking garak | ⚠️ Minor but classic: "every tool names the same secret differently. One scoped key, three env-var spellings (`ANTHROPIC_API_KEY`, `NVIDIA_API_KEY`, `NIM_API_KEY`) — map them explicitly at the boundary." |
